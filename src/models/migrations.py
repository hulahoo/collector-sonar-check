...
def create_migrations():
    with SyncPostgresDriver().session() as db:
        ...
        # create enums
        db.execute("create type update_status_enum as enum ('ENABLED', 'DISABLED', 'UPDATE_ERROR');")
        db.execute("create type auth_type as enum ('NAU', 'API', 'BSC')")
        db.execute("create type format_of_feed as enum ('CSV', 'JSON', 'XML', 'TXT')")
        db.execute("create type types as enum ('FEMA', 'SEMA', 'MD5H', 'SHA1', 'SHA2', 'IPAD', 'URLS', 'DOMN', 'FILE', 'REGS')")
        db.execute("create type format as enum ('STIX', 'MISP', 'FREE_TEXT', 'JSON', 'CSV')")
        ...
        # create statistic
        db.execute('create table "statistic" ("id" serial not null primary key, "created_at" timestamp not null, "data" jsonb default null);')
        ...
        db.execute('CREATE INDEX "intelhandler_indicator_tag_indicator_id_1dad0ba7" ON "indicator_tag_m2m_table" ("indicator_id");')
        db.execute('CREATE INDEX "intelhandler_indicator_tag_tag_id_2033539f" ON "indicator_tag_m2m_table" ("tag_id");')
        db.execute('CREATE INDEX "intelhandler_is_archived_attribute_mispobject_id_37232155" ON "indicator" ("is_archived");')

        # create parsing_rule
        db.execute('create table "parsing_rule" ("id" serial not null primary key,  "created_at" timestamp not null);')
        ...
        db.execute('CREATE UNIQUE INDEX "feed_indicator_m2m_table_41251221" ON "feed_indicator_m2m_table" ("indicator_id", "feed_id");')
